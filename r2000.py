...
sensor_image = ImageTk.PhotoImage(file="img/r2000.png")
...
request_handle_bt = ttk.Button(network, text="request_handle",width=20, padding=5, command=lambda: request_handle(http_commands('request_handle_udp?address='+ read_ip()[1] + '&port=6464&packet_type=C'))).grid(column=0,row=9)
...
rawdata_bt = ttk.Button(network, text="view_raw_data",width=20, padding=5, command=lambda: socket_connect(read_ip()[1])).grid(column=0,row=13)
